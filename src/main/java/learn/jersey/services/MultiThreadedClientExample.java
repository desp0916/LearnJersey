 ...
 * multi-threaded environment. Each table is a light weight object that is
 * created and thrown away. Connections are heavy weight objects that hold on to
 * zookeeper connections, async processes, and other state.
 *
 * <pre>
* Usage:
 ...
 *
 * <p>
 * The table should already be created before running the command. This example
 * expects one column family named d.
 * </p>
 * <p>
 * This is meant to show different operations that are likely to be done in a
 * real world application. These operations are:
 * </p>
 *
 * <ul>
 * <li>30% of all operations performed are batch writes. 30 puts are created and
 * sent out at a time. The response for all puts is waited on.</li>
 * <li>20% of all operations are single writes. A single put is sent out and the
 * response is waited for.</li>
 * <li>50% of all operations are scans. These scans start at a random place and
 * scan up to 100 rows.</li>
 * </ul>
 *
 */
public class MultiThreadedClientExample extends Configured implements Tool {
	...
	public MultiThreadedClientExample() throws IOException {
		...
		ThreadFactory threadFactory = new ThreadFactoryBuilder().setDaemon(true).setNameFormat("internal-pol-%d")
				.build();
	...
	public static class WriteExampleCallable implements Callable<Boolean> {
		...
		public Boolean call() throws Exception {
			...
			// Table implements Closable so we use the try with resource
			// structure here.
			...
			try (Table t = connection.getTable(tableName)) {
				...
				// now that we've assembled the batch it's time to push it to
				// hbase.
				t.put(puts);
			}
			return true;
	...
	public static class ReadExampleCallable implements Callable<Boolean> {
		...
		public Boolean call() throws Exception {
			...
			try (Table t = connection.getTable(tableName)) {
				...
				// This filter will keep the values from being sent accross the
				// wire.
				// This is good for counting or other scans that are checking
				// for
				...
				// Don't use the cache. While this is a silly test program it's
				// still good to be
				...
				try (ResultScanner rs = t.getScanner(s)) {
					...
					// Now go through rows.
					for (Result r : rs) {
						// Keep track of things size to simulate doing some real
						// work.
